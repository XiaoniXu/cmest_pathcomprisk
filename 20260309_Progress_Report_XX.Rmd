...
In the most recent phase of the project, I focused on validating the newly developed `cmcrhazard` R package by comparing its results against the original mediation analysis script (`250422_SHS_gfor_single_exp.R`). This comparison led to a critical bug fix and the implementation of a new "Exact Match" mode in the package.

Key updates include:
- **Bug Fix**: Identified and resolved an ordering issue in the bootstrap loop where covariate-coefficient mismatching occurred due to duplicate IDs.
- **New Feature: Exact Match Mode**: Implemented the `refit = TRUE` parameter in `cmest_pathcomprisk`, allowing for a full non-parametric bootstrap that matches the original script's logic.
- **Model Validation**: Performed a head-to-head comparison of the original re-fitting method versus the package's default parametric approach.
